...
screen_df <- dotplot_df %>% 
  select(gene, cluster, High_pval, High_beta, Low_pval, Low_beta, TFiMini_pval) %>% 
...
screen_df <- dotplot_df %>% 
  select(gene, cluster, High_pval, High_beta, Low_pval, Low_beta, TFiMini_pval, TFiMini_beta) %>% 
  pivot_longer(c(High_beta, Low_beta), names_to = "comp", values_to = "beta") %>% 
  pivot_longer(c(High_pval, Low_pval), names_to = "comp2", values_to = "pval") %>% 
  mutate(comp = str_extract(comp, "[A-Za-z]*"), comp2 = str_extract(comp2, "[A-Za-z]*")) %>% 
  filter(comp == comp2) %>% 
...
fix <- set_panel_size(screen_dotplot, height = unit(5, "cm"), width = unit(1.5, "cm"))
grid.arrange(fix)
ggsave("./output_files/FigE6c_dotplot_screen_REP.pdf", fix, dpi = 300,
       useDingbats=FALSE) 
...
fix <- set_panel_size(anova_dotplot, height = unit(5, "cm"), width = unit(1.5, "cm"))
grid.arrange(fix)
ggsave("./output_files/FigE6c_dotplot_anova_REP.pdf", fix, dpi = 300,
       useDingbats=FALSE)   

#Script to analyze QC qPCR for the TFiMini Screen
library(tidyverse)
library(scales)
library(egg)
library(gridExtra)
...
raw <- read_xls("./input_files/TFiMini_qPCR.xls", sheet = "Results", skip = 40) %>%
...
#Set y axis breaks and limits for each facet
y_list <- list(scale_y_continuous(expand = c(0,1), breaks = c(-1, -3, -5), name = "Relative expression [log2]"),
               scale_y_continuous(expand = c(0,1), breaks = c(-2, -5, -8), name = "Relative expression [log2]"),
               scale_y_continuous(expand = c(0,1), breaks = c(-2, -6, -10), name = "Relative expression [log2]"))
...
qpcr_plot <- plot_df %>% 
  ...
  scale_color_manual(values = c("#666666", "#90134D", "#C6C6C6"))

fix <- set_panel_size(qpcr_plot, height = unit(2, "cm"), width = unit(1.5, "cm"))
grid.arrange(fix)
ggsave("./input_files/FigE3c_TFiMini_qPCR.pdf", fix, dpi = 300,
       useDingbats=FALSE)

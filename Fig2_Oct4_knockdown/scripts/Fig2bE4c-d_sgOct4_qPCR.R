...
#Plot relative expression values for main figure (Xist and Oct4)
y_list_diff <- list(scale_y_continuous(expand = c(0,1), breaks = c(-1, 0, 1), limits = c(-1, 2)),
                    scale_y_continuous(expand = c(0,1), breaks = c(-8, -4, 0), limits = c(-9, 2)))


main_plot_df <- output %>% 
  separate(Sample, c("rep", "day", "med", "dTAG", "guide"), sep = "_") %>% 
  pivot_longer(-c(1:5), names_to = "Gene", values_to = "Rel_Exp") %>% 
  filter(Gene %in% c("Xist", "Oct4")) %>% 
  filter(dTAG == "KD") %>% 
  na.omit() 


#Calculate significance for main plot
test_main_df <- expand.grid(med = unique(main_plot_df$med), Gene = unique(main_plot_df$Gene))
test_main_df$pval <- mapply(function(a,b)
{t.test(main_plot_df[main_plot_df$guide=="TS14" & main_plot_df$Gene==a
                     & main_plot_df$med==b & main_plot_df$dTAG=="KD",]$Rel_Exp,
        main_plot_df[main_plot_df$guide=="LR15" & main_plot_df$Gene==a
                     & main_plot_df$med==b & main_plot_df$dTAG=="KD",]$Rel_Exp, 
        var.equal = TRUE)$p.value},
  test_main_df$Gene, test_main_df$med)

test_main_df$pval <- format(test_main_df$pval, scientific = FALSE)

main_plot <- main_plot_df %>% 
  ggplot(aes(x = med, y = Rel_Exp, fill = guide)) +
  ...
  stat_summary(geom = "crossbar", fun = "mean", width = 0.5, lwd = 0.25, position = position_dodge(width = 0.5),
               color = "black") +
  facetted_pos_scales(y = y_list_diff) +
  scale_fill_manual(values = c("#666666", "#F7A81F"))


fix <- set_panel_size(main_plot, height = unit(3, "cm"), width = unit(3, "cm"))
grid.arrange(fix)
ggsave("./output_files/Fig2b_sgOct4_qPCR.pdf", fix, dpi = 300,
       useDingbats=FALSE)
...
fix <- set_panel_size(lfc_d0_plot, height = unit(2, "cm"), width = unit(1.5, "cm"))
grid.arrange(fix)
ggsave("./output_files/FigE4c_sgOct4_qPCR_d0.pdf", fix, dpi = 300,
       useDingbats=FALSE)
...
grid.arrange(fix)
ggsave("./output_files/FigE4d_sgOct4_qPCR_diff.pdf", fix, dpi = 300,
       useDingbats=FALSE)

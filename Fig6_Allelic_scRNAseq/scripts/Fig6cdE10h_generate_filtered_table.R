...
#Calculate Pvalue for xist expression

test_df <- data.frame(cont = c("WT"), 
                      sample = c("dFtx-Xert"))

test_df$pval <- mapply(function(a, b) {
  wilcox.test(xist_df[xist_df$sample==a,]$Xist, xist_df[xist_df$sample==b,]$Xist)$p.value
}, test_df$cont, test_df$sample)
...
ggsave("./output_files/FigE10h_Xist_scatter.pdf", fix_scatter, 
       useDingbats=FALSE)

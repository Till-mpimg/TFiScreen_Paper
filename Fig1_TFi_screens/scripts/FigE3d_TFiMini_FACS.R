...
fcs_dir <- "./input_files/TFiMini_FCS/"
...
help_df <- flow_df %>% 
  ...
  mutate(`2i` = Xist, TFiMini = Xist, sgNT = Xist, sgXist = Xist) %>% 
  select(-Xist, -sample) %>% 
  pivot_longer(-1, values_to = "Xist", names_to = "sample")

#Plot TFi screen and controls as histograms
hist_plot <- flow_df %>% 
  filter(sample != "noProbe") %>%  
  ggplot() +
...
grid.arrange(fix)
ggsave("./output_files/FigE3d_TFiMini_FACS.pdf", fix, dpi = 300,
       useDingbats=FALSE)

#Plots rank plot for TFi Library (High/Negative)
library(tidyverse)
library(egg)
library(gridExtra)
library(ggrepel)
...
non_tf <- c("Xert", "Dcp1a", "Tsix", "Rif1", "Brd4", "Spen", "Dnmt1", "Trim28", "Kat8", "Eed", "Chd8",   
            "Msl1", "Msl2", "Kansl1", "Kansl3",  "Lbr", "Hnrnpu",  "Hnrnpk", 
            "Jpx", "Xist", "Ftx",  "Nexmif", "Gm9785", "Nup62cl", "Tslrn1", "Ripply1", "Eras", "Bcor", "Stag2", "Map3k15", 
            "Mid2", "Bcorl1",  "Rlim")
high_conf <- c("Xert", "Tsix", "Rif1", "Spen", "Brd4", "Msl1", "Msl2", "Kat8", "Jpx", "Ftx", "Rlim")

#Load High/Negative mle input
mle <- read.delim("./input_files/TFi_mle_HvN.gene_summary.txt") %>% 
  ...
  mutate(identity = ifelse(gene %in% high_conf, "cont", 
                           ifelse(pval >= 0.05, "ns", ifelse(gene %in% non_tf, "cont",
...
rank_plot <- mle %>% 
  ...
  geom_text_repel(data = mle[mle$rank >= 906,], aes(label = gene, color = identity), size = 6/2.8) +
...
fix <- set_panel_size(rank_plot, height = unit(4, "cm"), width = unit(5, "cm"))
grid.arrange(fix)
ggsave("./output_files/Fig1b_TFi_rank.pdf", fix, dpi = 300,
       useDingbats=FALSE)
